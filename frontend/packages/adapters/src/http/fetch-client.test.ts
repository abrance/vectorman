...
describe("FetchHttpClient", () => {
  ...
  it("calls global fetch with Window as this", async () => {
    const session = new MemoryAuthSession();
    const inner = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => new Response("{}", { status: 200 }));
  ...
  it("sends FormData without JSON content-type", async () => {
    const session = new MemoryAuthSession();
    const fetchImpl = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => new Response(JSON.stringify({ file_id: "f1" }), { status: 201 }));
    const client = new FetchHttpClient(session, new JsonErrorMapper(), fetchImpl as unknown as typeof fetch);
