//! VictoriaMetrics 远程适配器（v1 占位）。后续实现 TimeSeriesStore。
...
fn unimplemented_store() -> DataplaneError {
    DataplaneError::new(
        ...
        "dataplane-adapter-victoriametrics: not implemented in v1",
    )
}

/// VictoriaMetrics 远程时序存储（v1 占位）。
#[derive(Debug, Clone, Default)]
pub struct VictoriaMetricsStore;

#[async_trait]
impl TimeSeriesStore for VictoriaMetricsStore {
