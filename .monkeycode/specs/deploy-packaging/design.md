...
1. `VECTORMAN_VERSION=<REL> cargo build --release --workspace --target x86_64-unknown-linux-musl`（静态链接，不依赖目标机 glibc；需 `musl-tools` + `rustup target add`。`CC_x86_64_unknown_linux_musl=musl-gcc` 编译 C 依赖；不要把 `CARGO_TARGET_X86_64_UNKNOWN_LINUX_MUSL_LINKER` 设成 musl-gcc，ring/ureq 二进制会 SIGSEGV。`VECTORMAN_VERSION` 写入各二进制 `--version`）
