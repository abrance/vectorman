...
- [x] 5.9 进程指标的服务维度归一（实现期新增子项）
  ...
  - [x] **服务端部分已完成**：`MetricSink` 钩子（`dataplane-ingest/src/metric.rs`）→ `ApmSink` 实现
    按 `process_name`/`process_prefix` 查静态映射，填 `service`，未命中填 `unknown-<process_name>`。
...
- [ ] 6. 检查点 - P1 在特权 runner 上跑通受控流量用例后再进入 P2
  - 确保所有测试通过,如有疑问请询问用户
- [ ] 7. P1 前端与 CLI
  - [x] 7.1 `/ebpf` 页「边」视图：过滤、表格列、手动刷新、降级标记、前置校验状态
    - 对应需求 15.2-15.4、15.7-15.9；图表**复用仓库既有的手绘 SVG**（不引 echarts，与 APM/拓扑页一致；2026-09-25 修正）
    - 状态：已实现（PR #55）：`pages/ebpf-page.tsx` 的「边」页签；过滤含源/目标服务、协议、目标端口与来源（仅 eBPF / 两路合并 / 仅 OTLP）；
      表格列含时间、源/目标、目标地址、协议、连接数、失败数、双向字节、平均时长、重传、来源；进页面自动拉一次。
      **图表的偏差**：设计里写 echarts，实际沿用仓库既有的手绘 SVG 方案（与 APM/拓扑页一致，不新增依赖）；
      本页当前只做表格，边指标曲线仍在 `/topology` 与 `/apm` 页
  - [x] 7.2 `/ebpf` 页「事件」视图：`event_type`/`pid`/`process_name`/关键词过滤
    - 对应需求 15.3
    - 状态：已实现（PR #55）：事件页签支持类型/进程/关键词过滤，并显式提示「原始事件默认关闭、按比例抽样」（为空是常态而不是故障）
  - [x] 7.3 `/topology` 页 `source` 切换（全部 / otlp / ebpf）
    - 对应需求 15.5、11.4-11.5
    - 状态：已实现（PR #43 提供切换按钮，PR #53 起「全部」走服务端两路合并）；边行新增的 eBPF 字段（目标地址/字节/重传）在拓扑页不展示，统一在 `/ebpf` 页看
  - [ ] 7.4 边行「查看该边 trace」跳转 `/traces`
    - 对应需求 15.6
    - 状态：**部分实现**。拓扑页已有「按该边过滤 trace」的跳转；`/ebpf` 页的 eBPF 边**没有 trace 可跳**（eBPF 只给边不给 trace），
      因此该页只在未识别服务上提供「建立映射」入口。需求 15.6 的语义需按来源区分，已在设计里澄清
