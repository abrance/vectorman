...
3. THE 用户态 SHALL 每 `flush_interval_secs` 读取一次 map 快照；**读取到的即本周期增量**
   （读取后由用户态把内核侧计数写零复位），SHALL NOT 再与上一周期快照相减。
   复位失败时该周期 SHALL 不产出（数据留待下个周期读走），从而既不重复计数也不丢弃。
   _（2026-09-25 修正：原文写的是「与上一周期差分得到增量」，与该复位机制叠加会造成两次扣减 ——
   实测把每轮 80 次 `openat` 报成 1 次；修正原因见 `design.md` 的口径修正一节。）_
