...
每信号一个采集循环（`tokio` 任务），异常退出由装配层重建：

1. 每 `flush_interval_secs`（默认 10 秒）遍历 map 的**全部** CPU 副本，按键求和；
2. **读到的就是本周期增量**：`MapSource::drain*` 在读完内核 map 后会把计数**写零复位**，
   因此用户态**不再与上周期相减**（下面「口径修正」一节记录了重复扣减的 bug）。
   复位失败时 `drain` 返回 `Err` → 本周期跳过，数据留在内核 map 里等下一轮读走，
   既不重复计数也不丢弃。
...
### 容器/Pod 反查：按 `pid` 而不是 `cgroup_id`（实现期修正）

需求 11.1 说「由 `cgroup_id` 反查容器与 Pod」。实现改为**按 `pid`** 反查，理由：

- 内核给的 `cgroup_id` 是 cgroup 目录的 inode；映射到路径需要遍历 `/sys/fs/cgroup` 并逐目录 `stat`，
  cgroup v1 还要区分控制器层级（实测本机就是 v1/hybrid），成本高且脆弱；
- 连接键与进程键里**已经带了 `pid`**（`bpf_get_current_pid_tgid`），读 `/proc/<pid>/cgroup` 就能拿到
  同一条路径，且同一 pid 只查一次并缓存（TTL 5 分钟，上限 8192，满了整体清空）。

代价：进程已退出时 `/proc/<pid>` 不存在 → 拿不到容器信息（按空处理，**不缓存空结果**，避免 pid 复用后污染）。
收益：`process_include`/`process_exclude` 过滤第一次真正生效 —— 之前拿不到进程名，带进程过滤的采集项会把
所有记录都丢掉。

### Pod 名反查：k8s 索引 + 注入式加载器（实现期补充）

`/proc/<pid>/cgroup` 只能给出 Pod **uid**，而 dataserver 的端点表是按 Pod **名**登记的，所以
`src_pod` 直接用 uid 时「按 Pod 名命中端点表」这一层永远命中不了。补法是给反查器注入一个
**索引加载器**（`cgroup::PodNameLoader`：一次全量 `uid → name`）：

- 加载器由 **Agent 侧**（`collect/ebpf.rs`）构造，复用采集项配置里既有的 `namespace`/`kubeconfig`
  两个字段（与 `log_k8s_stdout` 同一份，**不新增配置面**）；
- **只在真正遇到 Pod uid 且索引过期时才拉**（TTL 5 分钟）：宿主机进程没有 Pod uid，不触发；
  同一 uid 的第二个 pid 复用同一份索引，不重复打 apiserver；
- 拉取失败**不带崩采集、也不清空旧索引**（旧名字比没有名字有用），只把加载时刻推后，
  避免每个 pid 都打一次注定失败的请求；
- 凭据在采集项启动时**先探一次**，拿不到就关闭反查并记一行日志（避免无 k8s 机器的日志噪音）；
- **退回语义**：名字拿不到时 `src_pod` 仍是 uid，与「未启用反查」的行为完全一致，不是回归。

命名空间为空时列**全部命名空间**（`GET /api/v1/pods`）：eBPF 采集面向整机，不该被单个命名空间限制。

边界：本机没有 k8s 集群，因此该链路用「假 apiserver + 临时 kubeconfig」的用例覆盖（凭据 → HTTP →
索引 → 反查器补名），**未在真实集群上验证**。

### 口径修正：用户态不做差分（实现期，修掉一处系统性少计）

原设计写「按键求和得到本周期**绝对值** → 与上周期相减得到增量」，即假设内核计数在两次读取之间
**单调不减**；但 `MapSource::drain*` 的实现是「读完把该键的所有 CPU 副本写零复位」，
于是每轮读到的本身就是增量。两者叠加成**双重扣减**：第二轮的值 = 本轮增量 − 上轮增量。

实测（专用进程每 4 秒稳定做 ~80 次 `openat`）：第一轮 80，第二轮 **1**；修复后 80/81/80/81。
影响面覆盖 `ebpf_network`（连接/字节/耗时/直方图）、`ebpf_process`（exec/exit/fork）与新增的 syscall ——
**长期存在的连接键被严重少计**（长连接每轮的字节数与耗时被减成 0，此前被误读成「没有数据」）。

修法：删掉三处差分，保留「写零复位 + 失败即跳过」。这也是 `aggregate` 里不再提供 `diff` 的原因
（留着会被再次误用）。

### `ebpf_syscall` 的实现口径（P2 第一半）

| 项 | 取法 |
| --- | --- |
| 延迟 | 4 个 op × `sys_enter_*`/`sys_exit_*` 的时间差（`ns >> 10` → 微秒，偏差约 2.4%） |
| 错误码 | `ret < 0` 时取 `-ret` 作 errno，按 `(op, errno)` 计数（需求 6.3 的维度就是这两个） |
| 键 | `(pid, cgroup_id, op, comm)`：`comm` 进键，指标才能出 `process_name` |
| `ret` 偏移 | 来自 `sys_exit_*/format`；**四个 exit 必须一致**，否则整项拒绝（不一致说明结构变了） |
| `filename` 偏移 | 来自 `sys_enter_openat/format`；缺失时置 0，**只丢路径不丢延迟** |
| BTF | **不需要**：内核程序只读 tracepoint 缓冲与助手，不读内核结构体 |
| 慢调用 | 超过 `slow_threshold_micros` 才读路径（`bpf_probe_read_user_str_bytes`，截断 256 字节）并推 `SLOW_IO` |
| 路径进聚合 map？ | 不进。路径只在慢事件里出现，避免「默认上行全量路径」（需求 6.4） |
| `service` 维度 | Agent **不发**该标签（不知道全局服务表），由 `dataserver` 的 `MetricSink` 补 |

`SLOW_IO` 单独一个 RingBuf 而不是复用 `EVENTS`：路径 256 字节，塞进 `RawEvent` 会让每次
进程/连接事件的拷贝都变大（那些事件比慢调用频繁得多）。内核侧**就地**填 ringbuf 条目
（字段逐个写），避免 300+ 字节的结构体临时量触发 BPF 栈限制。

### 真实下发链路上的两处修正（实现期）

走通 GSE 下发那一跳后暴露了两个问题，都已修正：

1. **数据面注册后立刻探活（带短重试）**：探活循环是「先探再睡 30 秒」，而数据面通常在业务进程
   启动后才注册，导致最长 30 秒处于 `unknown`；这段时间 Agent 取不到上报地址，采集数据积压。
   现在注册成功即探活，失败按 5 次 × 2 秒重试（数据面往往正在启动，只探一次就判 `offline` 同样要等 30 秒）。
2. **上行缓冲淘汰必须可见**：`Buffer`（容量 1000 条）满时淘汰最旧，此前只有一行日志。
   eBPF 一次 flush 就是几百条边记录，盲窗叠加后实测 60 秒丢 2279 条、只入库 149 条，
   而所有指标看起来都正常。现在 `push` 返回淘汰条数 → `EbpfStats.buffer_dropped` →
   `agent_ebpf_buffer_dropped_total`。
